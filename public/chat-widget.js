(function() {
  ...
  // Get configuration from script attributes with Safari compatibility
  ...
  // Expose config for testing (in development mode)
  if (typeof window !== 'undefined') {
    window.zentriaConfig = config;
  }
  ...
  function createWidget() {
    ...
    widgetContainer.innerHTML = `
      <style>
        :root {
          --zentria-primary-color: ${config.primaryColor};
          --zentria-primary-color-hover: ${config.primaryColor}dd;
          --zentria-primary-color-alpha: ${config.primaryColor}aa;
        }
        ...
        .zentria-chat-button {
          ...
          height: 60px;
          border-radius: 50%;
          background: var(--zentria-primary-color) !important;
          border: none;
          cursor: pointer;
        ...
        /* Safari Mobile and iOS specific fixes */
        ...
        .zentria-chat-header {
          background: var(--zentria-primary-color);
          color: white;
          margin: 10px;
          border-radius: 12px;
          padding: 25px;
        ...
        .zentria-message-content a {
          color: var(--zentria-primary-color);
        ...
        .zentria-send-button:hover {
          background: var(--zentria-primary-color-hover);
          transform: scale(1.05);
        }
  ...
  function updateWidgetAppearance() {
    ...
    root.style.setProperty('--zentria-primary-color', config.primaryColor);
    root.style.setProperty('--zentria-primary-color-hover', config.primaryColor + 'dd');
    root.style.setProperty('--zentria-primary-color-alpha', config.primaryColor + 'aa');
  ...
  function zentriaAddMessage(content, type) {
    ...
    // Colors are handled by CSS variables now
  ...
  // Global function to update appearance (for testing)
  window.zentriaUpdateAppearance = function() {
    updateWidgetAppearance();
  };

  // Initialize widget when DOM is ready with Safari compatibility
  function initializeWidget() {
    console.log('Initializing Zentria chat widget...');
    ...
    try {
      ...
      loadChatbotConfig().then(() => {
        ...
        // Add a delay to ensure DOM is fully ready, especially on Safari
        setTimeout(() => {
          initializeWelcomePopup();
        }, 300);
      }).catch(error => {
        console.warn('Failed to load chatbot config:', error);
        // Still initialize without config
        setTimeout(() => {
          initializeWelcomePopup();
        }, 300);
      });
  ...
  // Timing logic for Safari compatibility
